...
const getWeatherData = async function (cityName, lat, lon, APIKey) {
    const response = await fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=current,minutely,hourly,alerts&units=imperial&appid=${APIKey}`);
    ...
    currentTemp.innerHTML = `${Math.round(weatherData.daily[0].temp.day)} &#8457`;
    if (!isNaN(weatherData.daily[0].rain)) {
        currentPrecip.textContent = `${(weatherData.daily[0].rain / 25.4).toFixed(2)}"`;
    } else {
    ...
    currentDay.textContent = getWeekday(weatherData.daily[0].dt, {zone: weatherData.timezone});
...
const renderForcast = function (weatherArray, zone) {
    ...
    weatherArray.forEach(function(value, index) {
        if (index != 0 && index != 7) {
            ...
            if (!isNaN(value.rain)) {
                precipitation.innerHTML += ` ${(value.rain / 25.4).toFixed(2)}"`;
            } else {
            ...
            const wrapperChildren = [time, condition, temp, precipitation, windSpeed, humidity];
...
const getCities = async function (search, limit, APIKey) {
    const response = await fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${search}&limit=${limit}&appid=${APIKey}`);
...
searchResults.addEventListener('click', function (event) {
    ...
    if (event.target.tagName.toLowerCase() === 'li') {
        ...
        modalWindow.classList.add('hidden');
        getWeatherData(cityName, lat, lon, APIKey);
...
const findDupe = function (array) {

    const dupeNumbers = [];
    array.forEach(function(value, index, array) {
        if (indexOf(array) > 1 ) {
            dupeNumbers.push(value);
        }
    })
}